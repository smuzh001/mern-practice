...
Model.prototype.$__handleSave = function (options, callback) {
  ...
  if (this.isNew) {
    ...
    if (delta) {
      ...
      this.constructor.exists(this.$__where(), saveOptions).then(documentExists => {
    ...
    if (get(options, 'safe.w') !== 0 && get(options, 'w') !== 0) {
      ...
      if (this.$__.version && !this.$__.inserting) {
        ...
        if (numAffected <= 0) {
          ...
          const err = this.$__.$versionError || new VersionError(this, version, modifiedPaths);
          return callback(err);
...
Model.init = function init(callback) {
  ...
  const autoCreate = this.schema.options.autoCreate == null ? this.db.config.autoCreate : this.schema.options.autoCreate;
  ...
  return promiseOrCallback(callback, cb => {
    ...
    this.listIndexes((err, indexes) => {
      ...
      for (const index of indexes) {
        let found = false; // Never try to drop `_id` index, MongoDB server doesn't allow it
...
function isIndexEqual(model, schemaIndex, dbIndex) {
  ...
  const options = _decorateDiscriminatorIndexOptions(model, utils.clone(schemaIndex[1])); // If these options are different, need to rebuild the index
...
Model.listIndexes = function init(callback) {
  ...
  callback = this.$handleCallbackError(callback);
  return promiseOrCallback(callback, cb => {
    cb = this.$wrapCallback(cb); // Buffering
...
const safeDeprecationWarning = 'Mongoose: the `safe` option for `save()` is ' + 'deprecated. Use the `w` option instead: http://bit.ly/mongoose-save';
  ...
  return promiseOrCallback(cb, cb => {
    ...
    args.forEach(doc => {
      toExecute.push(callback => {
        const Model = this.discriminators && doc[discriminatorKey] != null ? this.discriminators[doc[discriminatorKey]] || getDiscriminatorByValue(this, doc[discriminatorKey]) : this;
...
Model.$__insertMany = function (arr, options, callback) {
  ...
  const toExecute = arr.map(doc => callback => {
...
Model.populate = function (docs, paths, callback) {
  ...
  const _this = this; // normalized paths
...
function _execPopulateQuery(mod, match, select, assignmentOpts, callback) {
  ...
  const query = mod.model.find(match, select, queryOptions); // If we're doing virtual populate and projection is inclusive and foreign
...
function _assign(model, vals, mod, assignmentOpts) {
  ...
  const rawDocs = {};
  let key;
  let val; // Clone because `assignRawDocsToIdStructure` will mutate the array

  const allIds = utils.clone(mod.allIds); // optimization:
...
Model.compile = function compile(name, schema, collectionName, connection, base) {
  ...
  if (typeof name === 'function' && name.prototype instanceof Model) {
    ...
    model = function model(doc, fields, skipId) {
      ...
      const Discriminator = model.discriminators[doc[discriminatorKey]] || getDiscriminatorByValue(model, doc[discriminatorKey]);
  ...
  const _userProvidedOptions = schema._userProvidedOptions || {}; // `bufferCommands` is true by default...


  let bufferCommands = true; // First, take the global option
...
Model.__subclass = function subclass(conn, schema, collection) {
  ...
  const s = schema && typeof schema !== 'string' ? schema : _this.prototype.schema;
