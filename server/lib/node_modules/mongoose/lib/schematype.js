...
function SchemaType(path, options, instance) {
  ...
  for (const prop of keys) {
    ...
    if (utils.hasUserDefinedProperty(this.options, prop) && typeof this[prop] === 'function') {
      ...
      const val = options[prop]; // Special case so we don't screw up array defaults, see gh-5780
...
const handleIsAsync = util.deprecate(function handleIsAsync() {}, 'Mongoose: the `isAsync` option for custom validators is deprecated. Make ' + 'your async validators return a promise instead: ' + 'https://mongoosejs.com/docs/validation.html#async-custom-validators');
/**
...
SchemaType.prototype.required = function (required, message) {
  ...
  this.requiredValidator = function (v) {
    const cachedRequired = get(this, '$__.cachedRequired'); // no validation when this path wasn't selected in the query.
...
SchemaType.prototype.doValidate = function (value, fn, scope, options) {
  let err = false;
  const path = this.path; // Avoid non-object `validators`
...
SchemaType.prototype.doValidateSync = function (value, scope, options) {
  ...
  validators.forEach(function (v) {
    ...
    validatorProperties.path = options && options.path ? options.path : path;
    validatorProperties.value = value;
    let ok; // Skip any explicit async validators. Validators that return a promise
