...
process.version.substr(1).split('.')[0] >= 6 && tape('indexOf latin1 and binary encoding', t => {
  const b = new BufferList('abcdef'); // test latin1 encoding
  ...
  t.equal(new BufferList(Buffer.from('aa\u00e8aa', 'latin1')).indexOf('\u00e8', 'latin1'), 2);
  t.equal(new BufferList(Buffer.from('\u00e8', 'latin1')).indexOf('\u00e8', 'latin1'), 0);
  t.equal(new BufferList(Buffer.from('\u00e8', 'latin1')).indexOf(Buffer.from('\u00e8', 'latin1'), 'latin1'), 0); // test binary encoding
  ...
  t.equal(new BufferList(Buffer.from('aa\u00e8aa', 'binary')).indexOf('\u00e8', 'binary'), 2);
  t.equal(new BufferList(Buffer.from('\u00e8', 'binary')).indexOf('\u00e8', 'binary'), 0);
  t.equal(new BufferList(Buffer.from('\u00e8', 'binary')).indexOf(Buffer.from('\u00e8', 'binary'), 'binary'), 0);
  t.end();
});
tape('indexOf the entire nodejs10 buffer test suite', t => {
  ...
  t.throws(() => b.indexOf('bad', 'enc'), TypeError); // test hex encoding
  ...
  {
    const twoByteString = Buffer.from('\u039a\u0391\u03a3\u03a3\u0395', 'ucs2'); // Test single char pattern
    ...
    t.equal(-1, index, `Not beta - at index ${index}`); // Test multi-char pattern
  ...
  const longBufferString = Buffer.from(longString); // pattern of 15 chars, repeated every 16 chars in long
  ...
  t.equal(1535, index, `Long JABACABA..., Second J - at index ${index}`); // Search for a non-ASCII string in a pure ASCII string.
  ...
  const allCharsBufferUcs2 = Buffer.from(allCharsString, 'ucs2'); // Search for string long enough to trigger complex search with ASCII pattern
  // and UC16 subject.
