...
test('should throws when prohibited characters', () => {
  ...
  expect(() => saslprep('a\u007Fb')).toThrow(); // C.2.2 Non-ASCII control characters

  expect(() => saslprep('a\u06DDb')).toThrow(); // C.3 Private use

  expect(() => saslprep('a\uE000b')).toThrow(); // C.4 Non-character code points

  expect(() => saslprep(`a${chr(0x1fffe)}b`)).toThrow(); // C.5 Surrogate codes

  expect(() => saslprep('a\uD800b')).toThrow(); // C.6 Inappropriate for plain text

  expect(() => saslprep('a\uFFF9b')).toThrow(); // C.7 Inappropriate for canonical representation

  expect(() => saslprep('a\u2FF0b')).toThrow(); // C.8 Change display properties or are deprecated

  expect(() => saslprep('a\u200Eb')).toThrow(); // C.9 Tagging characters
